...
class VariablesTab(QWidget):
    ...
    def initUI(self):
        ...
        self.retrieve_k.valueChanged.connect(self.validateRerankK)
    ...
    def validateRerankK(self, value):
        """Ensure rerank_k is not greater than retrieve_k"""
        if hasattr(self, 'rerank_k'):
            if self.rerank_k.value() > value:
                self.rerank_k.setValue(min(self.rerank_k.value(), value))
                self.rerank_k.setMaximum(value)
