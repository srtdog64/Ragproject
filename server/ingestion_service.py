...
from rag.core.interfaces import Ingester
...
async def async_ingest_documents(
    ...
    logger.info(f"Starting async ingestion of {total_docs} documents")
    ...
    try:
        ...
        for i in range(0, total_docs, batch_size):
            ...
            logger.debug(f"Processing batch {i//batch_size + 1}: docs {i+1}-{batch_end}")
            ...
            if doc_objects:
                ...
                task_info.current_item = f"Processing {doc_objects[0].title}"
                ...
                try:
                    ...
                    if result.isOk():
                        ...
                        logger.info(f"Ingested batch: {chunks} chunks from {len(doc_objects)} docs")
                    else:
                        error = str(result.getError())
                        logger.error(f"Batch ingestion failed: {error}")
                    ...
                    logger.error(f"Exception during batch ingestion: {e}")
                    for doc in doc_objects:
                        failed_docs.append({
                            "id": doc.id,
                            "error": str(e)
    ...
    logger.info(f"Async ingestion completed: {processed_docs}/{total_docs} docs, {total_chunks} chunks")
